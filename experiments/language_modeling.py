...
def generate_responses_zs_vllm(prompts, args, max_tokens=512):
...
def generate_responses_ft_vllm(prompts, args):
    ...
    output_dir = f'{args.model}_{expt_name}_{args.modality}/'
    ...
    merged_model.save_pretrained(f"{args.model}_{args.expt_name}_{args.modality}_merged", safe_serialization=True)

    tokenizer = AutoTokenizer.from_pretrained(args.model, trust_remote_code=True)
    tokenizer.pad_token = tokenizer.eos_token
    tokenizer.padding_side = "right"

    tokenizer.save_pretrained(f"{args.model}_{args.expt_name}_{args.modality}_merged")

    # llm = LLM(model="meta-llama/Llama-2-7b-hf", enable_lora=True, max_lora_rank=peft_config.r)
    llm = LLM(model=f"{args.model}_{args.expt_name}_{args.modality}_merged",
