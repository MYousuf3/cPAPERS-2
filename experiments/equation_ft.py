# Fine tune llama for equation qa
...
def preprocess_function_equations(example):
    example['text'] = f'<s>[INST] {example["neighboring_equations"]} {example["question"]} [/INST] [ANS] {example["answer"]} [/ANS] </s>'
    return example

def preprocess_function_inference_equations(example):
    example['text'] = f'<s>[INST] {example["neighboring_equations"]} {example["question"]} [/INST]'
    return example
...
def train(args):
    dataset = load_dataset("json", data_files="./cPAPERS/dataset/train/equation_final.jsonl", split="train")

    if args.modality == 'equation':
        preprocess_function = preprocess_function_equations
...
def eval(args):
    ...
    eval_dataset = load_dataset("json", data_files=f"./cPAPERS/dataset/{split}/equation_final.jsonl", split="train")
    
    if args.modality == 'equation':
        preprocess_function_inference = preprocess_function_inference_equations
    elif args.modality == 'context':
    ...
    if args.vllm:
        ...
        # Evaluator.print_results2('equation')
...
def parse_args():
    ...
    parser.add_argument("--expt_name", default="equation_ft")
