...
def preprocess_function_caption(example):
    example['text'] = f'<s>[INST] {example["caption"]} {example["question"]} [/INST] [ANS] {example["answer"]} [/ANS] </s>'
    return example

def preprocess_function_inference_caption(example):
    example['text'] = f'<s>[INST] {example["caption"]} {example["question"]} [/INST]'
    return example


def preprocess_function_contexts(example):
    example['text'] = f'<s>[INST] {example["context"]} {example["question"]} [/INST] [ANS] {example["answer"]} [/ANS] </s>'
    return example


def preprocess_function_inference_contexts(example):
    example['text'] = f'<s>[INST] {example["context"]} {example["question"]} [/INST]'
    return example


def preprocess_function_references(example):
    example['text'] = f'<s>[INST] {example["references"]} {example["question"]} [/INST] [ANS] {example["answer"]} [/ANS] </s>'
    return example


def preprocess_function_inference_references(example):
    example['text'] = f'<s>[INST] {example["references"]} {example["question"]} [/INST]'
    return example
...
def train(args):
    dataset = load_dataset("json", data_files="./cPAPERS/dataset/train/figure_final.jsonl", split="train")

    if args.modality == 'caption':
        preprocess_function = preprocess_function_caption
...
def eval(args):
    ...
    eval_dataset = load_dataset("json", data_files=f"./cPAPERS/dataset/{split}/figure_final.jsonl", split="train")
    
    if args.modality == 'caption':
        preprocess_function_inference = preprocess_function_inference_caption
    elif args.modality == 'context':
    ...
    if args.vllm:
        ...
        # Evaluator.print_results2('figure')
...
def parse_args():
    ...
    parser.add_argument("--modality", help='modality to use -> figure, context, or reference', choices=['caption', 'context', 'reference'])
    ...
    parser.add_argument("--expt_name", default="figure_ft")
